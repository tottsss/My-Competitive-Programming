...
void operator += (BigInt &a, long long b){
    a = a + b;
}
...
BigInt O = Integer(1);
BigInt OZ = Integer(-1);
//BigInt f(int d, int s){
//	if(d==0 and s == 0) return O;
//	if(d <= 0 or s < 0 or s>d*9) return Z;
//	if(d ==1) return Z;
//	if(dp[d][s] != OZ) return dp[d][s];
//	BigInt ans = Z;
//	for(int dig = 0; dig <= min(s, 9); dig++){
//		ans += f(d - 1, s - dig);
//	}
//	return dp[d][s] = ans;
//}
//long long dp[101][101*9];
//long long f(long long d, long long s){
//	if(d==0 and s == 0) return 1LL;
//	if(d <= 0 or s < 0 or s>d*9) return 0LL;
//	if(d ==1) return 1LL;
//	if(dp[d][s] != -1) return dp[d][s];
//	long long ans = 0;
//	for(long long dig = 0; dig <= min(s, 9LL); dig++){
//		ans = ans + f(d - 1, s - dig) * 1LL;
//	}
//	return dp[d][s] = ans;
//}
main(){
	...
	int n; cin >> n;
	for(int i = 0;i <= 50; i++){
		for(int s = 0; s <= 50*9; s++) dp[i][s] = Z;
	}
	BigInt sum = Z;
	dp[0][0] = O;
	for(int i = 1;i <= n/2; i++){
		for(int dig = 0; dig < 10; dig++){
			for(int s = dig; s <= n/2*9; s++){
				dp[i][s]+= dp[i-1][s-dig];
			}
		}
	}
